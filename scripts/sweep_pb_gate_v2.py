"""Robustness sweep v2: serial execution, save to CSV."""
...
def _run_one(gate_val):
    ...
    for yr in [2019, 2020, 2018, 2022]:
        ...
        if len(sn) >= 2:
            ...
            yearly[f"ret_{yr}"] = 0; yearly[f"dd_{yr}"] = 0
    return {
        "gate": gate_val, "total_return": round(total_ret, 2),
        ...
        "pb_buys": pb_buys, "pb_2024": pb_2024,
        **yearly,
    }
...
if __name__ == "__main__":
    results = []
    t0 = time.time()
    for gv in GATE_VALUES:
        t1 = time.time()
        r = _run_one(gv)
        elapsed = time.time() - t1
        results.append(r)
        print(f"gate={r['gate']} ret={r['total_return']:.1f} dd={r['max_drawdown']:.1f} "
              f"pb={r['pb_buys']} pb24={r['pb_2024']} 19r={r['ret_2019']:.1f} "
              f"20r={r['ret_2020']:.1f} ({elapsed:.0f}s)")
    df = pd.DataFrame(results)
    print(f"\nTotal: {time.time()-t0:.0f}s")
    print(df.to_string(index=False))
    df.to_csv(ROOT / "data" / "diagnosis" / "gate_sweep.csv", index=False)
    print("\nA: ret=83.1 dd=-8.1 cal=0.97 trades=1231 no24=14.9 19=4.1 20=-1.6 18dd=-1.1 22dd=-6.2")
